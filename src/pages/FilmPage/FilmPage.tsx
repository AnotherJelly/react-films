...
export function FilmPage() {
  ...
  const filmId = id ? Number(id) : null;
  const getFavoriteById = useStore(state => state.getFavoriteById);
  const favFilm = filmId ? getFavoriteById(filmId) : null;
  const [apiFilm, setApiFilm] = useState<Film | null>(null);

  useEffect(() => {
    if (favFilm || !filmId) return;

    getFilmById(filmId).then((data) => {
      setApiFilm(data || null);
    });
  }, [filmId, favFilm]);

  const film = favFilm || apiFilm;
