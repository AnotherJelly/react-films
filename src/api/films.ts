import type { Film, FilmApi } from "../types/film";
...
function mapApiFilmToFilm(apiFilm: FilmApi): Film {
  return {
    id: apiFilm.id,
    title: apiFilm.name || 'Без названия',
    slogan: apiFilm.slogan || '',
    year: apiFilm.year || 0,
    ratingOverall: Number(apiFilm.rating?.kp.toFixed(1)) || 0,
    ...
    genres: apiFilm.genres?.map((g: { name: string }) => g.name) || [],
